...
def kl_similarity(vnr_a, vnr_b, substrate):
    """
    Calculate similarity using KL divergence between candidate distributions.
    
    For each vnode position, treat candidates as probability distributions
    over substrate nodes and compute symmetric KL (Jensen-Shannon) divergence.
    
    Args:
        vnr_a: First VNR graph
        vnr_b: Second VNR graph
...
# ============================================================
# Core MP-PVA (Cross-VNR) with Early Stopping - Using KL Similarity
# ============================================================
...
def run_algorithm(vnr_list, substrate_snapshot,
    ...
    for gen in range(generations):
        ...
        # --- Step each VNR ---
        for ctx in contexts:
            # Early Stopping: If already solved, skip
        ...
        # --- Cross-VNR Knowledge Transfer using KL Similarity ---
        for ctx in contexts:
            ...
            for d in donors:
                # Use KL divergence-based similarity instead of set intersection
                sim = kl_similarity(ctx.vnr, d.vnr, substrate_snapshot)
                scored_donors.append((sim, d))

            # Sắp xếp giảm dần (higher similarity = better donor)
